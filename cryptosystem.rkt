...
(define (key n)
  (let ((tuple (get-nth-quadruple n)))
    (let ((e (snd tuple)) (f (trd tuple)))
...
(define (message->codes message)
  (map (λ (x) (let ((c (char->integer x)))
                (if (= c 32)
                    0
                    (modulo c 96)))) (string->list message)))
...
(define (codes->message codes)
  (list->string (map integer->char (map (λ (x) (if (= x 0)
                                                   32
                                                   (+ 96 x))) codes))))
...
(define (extend-key key size)
  (let ((original key))
    (let iter ((key original) (result null) (current-size 0))
      (if (= current-size size)
          result
          (iter (if (= (length key) 1)
                    original
                    (cdr key)) (append result (list (car key))) (+ current-size 1))))))
...
(define (encrypt-codes message key)
  (apply-changes message (extend-key key (length message)) +))
(define (decrypt-codes message key)
  (apply-changes message (extend-key key (length message)) -))

(define (apply-changes code1 code2 func)
  (map (λ (x) (modulo x 27)) (map func code1 code2)))
